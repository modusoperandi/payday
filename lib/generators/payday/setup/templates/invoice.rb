class <%= options.invoice_name.singularize %> < ActiveRecord::Base
  ...
  has_one :<%= options.address_name.pluralize.underscore %>, :as => :billing_address
  has_one :<%= options.address_name.pluralize.underscore %>, :as => :shipping_address
  has_one :<%= options.address_name.pluralize.underscore %>, :as => :invoicing_party_address
  ...
  has_attached_file :pdf_document, :styles => { :thumb => { :geometry => "141x200>", :format => :jpg } }
